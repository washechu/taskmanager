...
### Даты у задач — только дедлайн

У задач **одно** date-поле — «Дедлайн» (`due_date`). Колонка `tasks.start_date` в БД осталась (не дропнута), но **в UI задач не используется** и обнулена (м.023). История: была «Начало» → побыла «Отложить до» (snooze) → убрана как лишнее поле в форме (ради редкого снуза висело при создании каждой задачи). У **проектов** `start_date` сохранён («начало проекта» для Ганта).

В Ганте задача без `start_date` (т.е. любая задача после м.023) рисуется **точкой-ромбиком на дате дедлайна** (`rangeOf(null, due) → isPoint`), а не полоской — задача с одним дедлайном это момент, а не диапазон.

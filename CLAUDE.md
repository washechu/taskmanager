...
│   ├── habits/
│   │   ├── HabitsWeek.tsx        # Карточки привычек + дни недели + навигация + streak
...
- **`HabitsWeek`** — **карточки** (а не таблица: с малым числом привычек таблица оставляла много пустого «воздуха»). Контейнер `max-w-2xl mx-auto`. Каждая карточка: цветная точка + название + 🔥streak, мини-прогресс-бар «N/M на неделе», ниже — кружки **только запланированных дней** этой недели (Пн→Вс, не все 7 — компактнее, без гориз. скролла на мобиле). Состояния дня (`DayCell`): выполнено (заливка цветом + ✓), сегодня-не-сделано (синяя обводка), **пропущено** (прошлый запланированный день — красный пунктир + «·», клик доотмечает задним числом), будущее (бледное, disabled). Навигация по неделям ←/→ + «Сегодня» (`weekStartsOn:1`).
- **Дальше (не сделано):** поле-иконка вместо эмодзи в названии, пауза/архив (колонка `archived` уже есть, UI нет), режим «Месяц»/тепловая карта, статистика (лучшая серия, % за месяц).

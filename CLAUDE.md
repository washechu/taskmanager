...
- **Модель:** `habits` (название, **иконка-эмодзи**, `weekdays int[]` ISO 1=Пн…7=Вс, цвет из палитры тегов, `archived`, `assignee`, `category`) + `habit_logs` (по строке на выполненный день). **Наличие строки в `habit_logs` = выполнено**, отдельного флага `done` нет — снятие отметки удаляет строку. `unique(habit_id, date)` защищает от дублей.
...
- **Тоггл «Все привычки / Сегодня»** в шапке страницы (на месте бывшего фильтра по ответственному). «Сегодня» оставляет только привычки, у которых сегодняшний ISO-день недели есть в `weekdays`. Состояние — `scope` в `page.tsx`; если по «Сегодня» ничего нет, в `HabitsView` через проп `emptyText` показывается «На сегодня привычек по расписанию нет».
...
- **`HabitsView`** — **карточки** (а не таблица: с малым числом привычек таблица оставляла много пустого «воздуха»). Контейнер `max-w-2xl mx-auto`. Два режима через тогглер **Неделя / Месяц** (навигация ←/→ шагает неделей/месяцем, `weekStartsOn:1`). Кнопка «Сегодня» возвращает к текущему периоду и **disabled, когда уже на нём** (`isSameWeek`/`isSameMonth`). Каждая карточка: цветная точка + название + описание (1 строка) + 🔥streak, мини-прогресс-бар.
  - **Неделя:** кружки **только запланированных дней** недели (не все 7 — компактнее, без гориз. скролла; на мобиле `h-9`, на десктопе `h-11`).
  - **Месяц:** grid 7×N — тепловая карта: запланированные дни = кружки с числом, незапланированные = бледное число, вне месяца — пусто.
  - Состояния дня (`DayCircle`): выполнено (заливка цветом + ✓), сегодня-не-сделано (синяя обводка), **пропущено** (прошлый запланированный — красный пунктир, клик доотмечает задним числом), будущее (бледное, disabled).
...
**Задачи:**
- Категория: Личное / Семейное (вкладки, дефолт `personal`). Нет «Все» — это намеренно: пользователь не смотрит «всё подряд», а работает в своём скоупе. Личное автоматически = свои задачи (через `currentUser.assignee`).
- Ответственный: select (только в категории «Семейное»; в «Личном» скрыт)
- Теги: мультиселект (chips из БД)
- Фильтра «Проект» нет — задачи проекта смотрят со страницы Проектов через модалку.

**Проекты:**
- Категория: вкладки Личное/Семейное (как у задач)

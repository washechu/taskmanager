...
У задач одно date-поле в форме — «Дедлайн» (`due_date`). История эры `start_date` в форме: было «Начало» → побыло «Отложить до» (snooze) → убрано (ради редкого снуза висело при создании каждой задачи).

С м.025 колонка `tasks.start_date` получила новую семантику — **фактическая дата начала работы**. Заполняется автоматически DB-триггером `_auto_set_start_date` (BEFORE INSERT OR UPDATE OF status):
- при первом переходе задачи в `in_progress` (или создании со статусом `in_progress`)
- если `start_date` пуст
- ставим `current_date`

Не перезаписываем при последующих переходах — дата первого старта работы сохраняется навсегда (вернули из done → не сбрасываем; in_progress→paused→in_progress → дата с первого раза). Старые задачи (обнулённые м.023) получают `start_date` только при следующем переводе в `in_progress`.

В UI: `TaskModal` показывает «Начало: YYYY-MM-DD» рядом с «Дедлайн» (только если есть). В форму не возвращаем — поле автоматическое.

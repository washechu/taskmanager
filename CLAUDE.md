...
- **Привычки индивидуальны:** категория в UI **не задаётся** (всегда `personal`, дефолт в БД с м.006), `assignee` автоматически = создатель (`currentUser.assignee`, передаётся в `HabitForm` как `defaultAssignee`, отдельного селекта нет). На странице фильтр только по ответственному (чтобы видеть свои/партнёра).
- **Иконка:** эмодзи из курируемого набора `HABIT_ICONS` (пикер в `HabitForm`), хранится в `habits.icon`. Если иконки нет — в карточке рисуется цветная точка. Цвет привычки при этом всё равно управляет заливкой выполненных дней и прогресс-баром.
...
- **`HabitsView`** — **карточки** (а не таблица: с малым числом привычек таблица оставляла много пустого «воздуха»). Контейнер `max-w-2xl mx-auto`. Два режима через тогглер **Неделя / Месяц** (навигация ←/→ шагает неделей/месяцем, `weekStartsOn:1`, кнопка «Сегодня»). Каждая карточка: иконка/точка + название + описание (1 строка) + 🔥streak, мини-прогресс-бар.
